...
class VisualFrameTest(VisualTest):
    def __init__(self, test_path, descr,*args):
        iframe = document.createElement('iframe')
        test_name = os.path.split(test_path)[1][:-3]
        title = f'<a href="test.html?s={test_path}" target="_blank" >{test_name}</a>'

        iframe.setAttribute('src', f"test.html?banner=false&s={test_path}" )
        iframe.setAttribute('width', 400 + 20)
        iframe.setAttribute('height', 400 + 70)
        iframe.style.overflowX = 'hidden'
        
        super().__init__(title, descr,  iframe, width=420, *args)
        self.test_box.style.minHeight = "540px"
        self.test_box.style.overflowX = "hidden"
        self.visual_wrapper.style.minHeight = "480px"
